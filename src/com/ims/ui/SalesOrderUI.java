...
public class SalesOrderUI extends IFrame {
	...
	public SalesOrderUI() {
		...
		lblFullName.setText("<html><body><u style=\"color:blue;\">"+Program.LANG.getProperty("lblcustname")+"</u></body></html>");
	...
	private class SalesOrderUIListener implements SuggestionSelectedListener,
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == cboProgCatg) {
				...
				int ret = order.createSalesOrder(mdata, data, custdata);
				if (ret == tblProdList.getRowCount() + 1) {
					Statics.showMessage(getContentPane(),
							"Sales order updated successfully!",
							JOptionPane.INFORMATION_MESSAGE);
					mdata=order.getLastOrder();
					Firm firm=new Firm();
					HashMap<String, Object> map = new HashMap<>();
					map.put("prmsalemid", mdata.somid);
					map.put("prmfirmid", firm.getFirmData().firmid);
					ReportViewer view = new ReportViewer("salesorder",
