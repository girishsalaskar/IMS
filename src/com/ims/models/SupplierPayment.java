...
public class SupplierPayment extends IMSDB {
	...
	public SupplierPayData getBillWisePayment(String billno, long supid)
	{
		SupplierPayData data=null;
		try {
			super.executeQuery("select * from supppayment where billno like ? and supid=?",
					billno, supid);
			if (resultset.next()) {
				data = new SupplierPayData();
