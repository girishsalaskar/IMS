...
public class Customers extends IMSDB {
	...
	public long quickSave(CustomersData data)
	{
		long custid=-1;
		try {
			int ret=0;
			ret=super.executeUpdate(
					"insert into customers(custname, custphone) values(?, ?)",
					data.custname, data.custphone);
			if(ret>0)
			{
				custid=getLastCustomer().custid;
			}
		} catch (SQLException | DatabaseNotConnectedException e) {
			Statics.handleException(e);
		}
	...
	public CustomersData getLastCustomer() {
		CustomersData data = null;
		try {
			super.executeQuery("select * from customers where custid in (select max(c.custid) from customers c)");
			if (resultset.next()) {
				data = new CustomersData();
				initData(data, resultset);
			}
