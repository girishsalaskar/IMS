...
public class SalesUI extends IFrame {
	...
	private void updateOrderDetails() {
		if (cboOrderId.getSelectedIndex() > 0) {
			...
			CustomersData cust = new Customers()
					.getCustomerById(somdata.custid);
	...
	private class SalesUIListener implements MouseListener, ActionListener,
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == cboOrderId) {
				...
				int ret = sales.makeSalesEntry(mdata, prods, newdr);
				if (ret < 1) {
					Statics.showMessage(getContentPane(),
					...
					Sales_MData[] smdata = sales.getSalesData(
							Long.parseLong(txtCustId.getText()),
							dpkSalesDate.getDate());
					map.put("prmsalemid", smdata[smdata.length - 1].salemid);
					Firm firm = new Firm();
