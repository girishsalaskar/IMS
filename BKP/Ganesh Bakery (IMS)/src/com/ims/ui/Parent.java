...
public class Parent extends MDIParent {
	...
	private JMenuItem mniCustRoute;
	...
	private JMenu mnuRouteMgt;
	private JMenuItem mnuRoutes;
	private JMenuItem mniRouteTrip;
	...
	private JMenuItem mniCustRpts;

	private JMenu mnuFirm;
	private JMenuItem mniFirmProf;
	private JMenuItem mniAssets;
...
//		mfont = new Font(Font.SERIF, Font.BOLD, 15);
		...
		mniCustRoute = new JMenuItem(Program.LANG.getProperty("mnicustroutes"));
		mniCustRoute.addActionListener(listen);
		mniCustRoute.setFont(mfont);
		mnuCustomer.add(mniCustRoute);
		...
		mnuRouteMgt = new JMenu(Program.LANG.getProperty("mnuroute"));
		mnuRouteMgt.setFont(mfont);
		mnuRouteMgt.setMnemonic('o');
		mnuRoutes = new JMenuItem(Program.LANG.getProperty("mniroutemaster"));
		mnuRoutes.addActionListener(listen);
		mnuRoutes.setFont(mfont);
		mnuRouteMgt.add(mnuRoutes);

		mniRouteTrip = new JMenuItem(Program.LANG.getProperty("mniroutetrip"));
		mniRouteTrip.addActionListener(listen);
		mniRouteTrip.setFont(mfont);
		mnuRouteMgt.add(mniRouteTrip);
		menubar.add(mnuRouteMgt);
		...
		mniCustRpts = new JMenuItem(Program.LANG.getProperty("mnicustrpt"));
		mniCustRpts.setFont(mfont);
		mniCustRpts.addActionListener(listen);
		mnuReports.add(mniCustRpts);
		...
		mniAssets = new JMenuItem(Program.LANG.getProperty("mniassets"));
		mniAssets.setFont(mfont);
		mniAssets.addActionListener(listen);
		mnuFirm.add(mniAssets);
		...
		toolbar.add(lblDev, BorderLayout.EAST);
		...
		try {
			...
			mnuRouteMgt.setIcon(new ImageIcon(ImageIO.read(Statics
					.getImageStream("truck.png"))));
		...
		Insets in = new Insets(0, 10, 0, 10);
				...
				if (JOptionPane.showConfirmDialog(getContentPane(),
					...
					if (WizardStarter.isDemo) {
						...
						try {
							...
							db.execute("INSERT INTO `users`(`userid`, `pwd`, `uautho`, `uname`,`mob`) values('girish', sha1('GirishNSGS'), 1, 'Girish', '9860911209')");
	...
	private void checkConfig() {
		FirmDetails_UI firm = new FirmDetails_UI();
		if (firm.getFirmID() < 1) {
	...
	private void enableMenus() {
		...
		mnuRouteMgt.setEnabled(isadmin);
	...
	private class ParentListener implements PropertyChangeListener,
		...
		@Override
		public void propertyChange(PropertyChangeEvent e) {// INFO Login Attempt
		...
		public void actionPerformed(ActionEvent e) {// INFO Action Listener
			...
			if (e.getSource() == mniChangePass) {
			...
			else if (e.getSource() == mnuRoutes) {
				if (!isFrameOpened(RouteMasterUI.class.getName(), true))
					addFrame(new RouteMasterUI());
				...
				ReportViewer view = new ReportViewer("stock.jrxml", map,
				...
				if (!isFrameOpened(EmpSalReport.class.getName(), true))
					addFrame(new EmpSalReport());
				...
				win.setBounds(100, 100, calc.getSize().width + 10,
						calc.getSize().height + 10);
				win.setVisible(true);
			} else if (e.getSource() == mniAssets) {
				if (!isFrameOpened(AssetsUI.class.getName(), true))
					addFrame(new AssetsUI());
			...
			} else if (e.getSource() == mniCustRoute) {
				if (!isFrameOpened(CustRoutesUI.class.getName(), true))
					addFrame(new CustRoutesUI());
			} else if (e.getSource() == mniRouteTrip) {
				if (!isFrameOpened(RouteTripUI.class.getName(), true))
					addFrame(new RouteTripUI());
			} else if (e.getSource() == mniShopStat) {
				if (!isFrameOpened(ShopStatus.class.getName(), true))
					addFrame(new ShopStatus());
			} else if (e.getSource() == mniCustRpts) {
				if (!isFrameOpened(CustReportUI.class.getName(), true))
					addFrame(new CustReportUI());
